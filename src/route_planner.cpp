...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    // populate current_node.neigbors vector wiat all neighbors
    current_node->FindNeighbors();

    // for each neighbor
    for (int i = 0; i < current_node->neighbors.size(); i++) {
        // set the parent, the h_value, and the g_value 
        ...
        // add neighbor to open_list and set the node's visited attribute to true
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    // create pointer to first entry of the list
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // backpropagate parent nodes until start_node is reached
    ...
    // add start_node to the beginning of path_found
    path_found.emplace(path_found.begin(), *(current_node));
...
void RoutePlanner::AStarSearch() {
    ...
    // add all neigbhors to open_list
    AddNeighbors(start_node);

    // flag start_node as visited
    start_node->visited = true;

    // TODO: Implement your solution here.

    // Search for the most cost-efficient path and return the final path
